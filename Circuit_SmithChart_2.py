"""RF Circuit Design Tool - Smith Chart Impedance Analyzer (v2)
Feature additions over rf_circuit_design.py:
  - Component.tolerance 속성 (개별 소자 tolerance %)
  - 더블클릭 편집 다이얼로그에 Tolerance QDoubleSpinBox 추가
  - 민감도 분석 (Local Sensitivity Analysis): SensitivityInputDialog,
    SensitivityResultWindow, compute_local_sensitivity()
"""
...
# ---------------------------------------------------------------------------
# 공통 버튼 스타일 상수 (Advanced / Sensitivity Analysis 공유)
# ---------------------------------------------------------------------------
...
class ComponentEditDialog(QDialog):
    ...
    def __init__(self, comp: Component, parent=None):
        ...
        self.val_spin.setRange(1e-9, 1e9)
        self.val_spin.setDecimals(4)
        self.val_spin.setValue(comp.value if comp.value is not None else 0.0)
        self.val_spin.setSuffix(f'  {unit}')
        form.addRow(f'값  ({unit})  {hint}:', self.val_spin)

        # Tolerance 입력 (QDoubleSpinBox + "±%" 레이블)
        tol_container = QWidget()
        tol_layout = QHBoxLayout(tol_container)
        tol_layout.setContentsMargins(0, 0, 0, 0)
        self.tol_spin = QDoubleSpinBox()
        self.tol_spin.setRange(0.0, 50.0)
        self.tol_spin.setSingleStep(0.5)
        self.tol_spin.setDecimals(1)
        self.tol_spin.setValue(comp.tolerance)
        tol_layout.addWidget(self.tol_spin)
        tol_layout.addWidget(QLabel('±%'))
        form.addRow('Tolerance:', tol_container)
...
# ---------------------------------------------------------------------------
# MNA 단일 주파수 솔버 (민감도 분석용 동기 함수)
# ---------------------------------------------------------------------------

def _mna_solve_for_sensitivity(freq_hz: float,
                                components: List[Component],
                                wires: List[Wire]) -> complex:
    """단일 주파수에서 복소 임피던스를 계산 (민감도 분석용 동기 MNA 솔버).
    CalcWorker._build_graph / _solve_mna 로직을 독립 함수로 추출.
    """
    try:
    ...
    for w in wires:
        union((w.start_comp_id, w.start_pin),
              (w.end_comp_id, w.end_pin))

    all_keys: set = {gnd_key, plus_key}
    for c in components:
    ...
    I_vec[0] = 1.0  # PORT plus 노드에 1A 주입
...
def compute_local_sensitivity(components: List[Component],
                               wires: List[Wire],
                               freq_hz: float) -> List[Dict]:
    """중심차분(central difference)으로 각 소자의 정규화/절대 민감도 계산.

    반환: |S_normalized| 내림차순 정렬된 dict 리스트
    각 dict: name, value, unit, tolerance, delta_x,
             S_normalized (부호 있음), S_absolute (부호 있음)
    """
    valid_comps = [c for c in components if c.value is not None]
    if len(valid_comps) <= 1:
        raise ValueError(
            "민감도 분석에는 값이 입력된 소자가 2개 이상 필요합니다.\n"
            "모든 소자의 값을 입력한 후 다시 시도하세요.")

    # 공칭 임피던스 계산
    Z_nominal = _mna_solve_for_sensitivity(freq_hz, components, wires)
    Z_nom_mag = abs(Z_nominal)
    if not np.isfinite(Z_nom_mag) or Z_nom_mag == 0:
        raise ValueError(
            f"공칭 임피던스 계산 실패 (Z = {Z_nominal:.4f}).\n"
            "회로가 올바르게 연결되어 있는지 확인하세요.")

    # 소자 유형별 순번 카운터 (이름 생성용)
    ...
    for comp in components:
        if comp.value is None:
            continue
        cnt = type_counters.get(comp.type, 0) + 1
        type_counters[comp.type] = cnt
        name = f'{comp.type}{cnt}'
        unit = {'R': 'Ω', 'L': 'nH', 'C': 'pF'}[comp.type]

        xi_nominal = comp.value
        tol_pct = comp.tolerance  # %
        warning = ''

        # Perturbation 크기 (중심차분)
        if xi_nominal == 0:
            delta = 1e-9
            warning = '*'
        else:
            delta = xi_nominal * (tol_pct / 100.0)
            if delta == 0:
                delta = 1e-9
                warning = '*'

        try:
            # 해당 소자의 값만 변경한 복사본 생성
            comps_plus  = copy.deepcopy(components)
            comps_minus = copy.deepcopy(components)
            for cp in comps_plus:
                if cp.id == comp.id:
                    cp.value = xi_nominal + delta
            for cm in comps_minus:
                if cm.id == comp.id:
                    cm.value = xi_nominal - delta

            Z_plus  = _mna_solve_for_sensitivity(freq_hz, comps_plus,  wires)
            Z_minus = _mna_solve_for_sensitivity(freq_hz, comps_minus, wires)

            # 스펙: dZ_dXi = (|Z_plus| - |Z_minus|) / (2 × δ)
            dZ_dXi = (abs(Z_plus) - abs(Z_minus)) / (2.0 * delta)
            # 정규화 민감도 (무차원, 부호 있음)
            S_normalized = dZ_dXi * (xi_nominal / Z_nom_mag)
            S_absolute   = dZ_dXi    # Ω/단위 (부호 있음)
    ...
    # |S_normalized| 내림차순 정렬 (NaN은 맨 뒤)
    results.sort(
        key=lambda r: (
            -abs(r['S_normalized'])
            if np.isfinite(r['S_normalized']) else float('inf')
        )
    )
    return results
...
class SensitivityInputDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.freq_spin.setValue(13.56)  # 기본값: 13.56 MHz
        self.freq_spin.setFixedWidth(110)
        ...
        run_btn.setDefault(True)  # 엔터 키 → Run Analysis
...
# ---------------------------------------------------------------------------
# 2단계 팝업 — 결과 창 (Non-modal)
# ---------------------------------------------------------------------------

class SensitivityResultWindow(QDialog):
    """민감도 분석 결과: 테이블(좌) + 토네이도 차트(우), 비모달."""

    def __init__(self, results: List[Dict], freq_hz: float, parent=None):
        super().__init__(parent)
    ...
    def _build_ui(self):
        ...
        # ── 상단 정보 표시 ───────────────────────────────────────────────
        z_nom_mag = float('nan')
        if results_have_data := any(
                np.isfinite(r['S_normalized']) for r in self.results):
            # Z_nominal은 따로 저장되지 않으므로 역산으로 추정
            pass
        freq_label = QLabel(
            f'분석 주파수: {self.freq_mhz:.3f} MHz   |   소자 수: {len(self.results)}개'
        )
        freq_label.setStyleSheet('font-size: 10pt; font-weight: bold;')
        root.addWidget(freq_label)
    ...
    def _build_table(self) -> QTableWidget:
        headers = ['Component', 'Value', 'Tolerance', '|ΔZ/ΔXi|', 'Norm. S', 'Rank']
        ...
        # 헤더 스타일: 굵게, 배경 #2d5986, 흰색 텍스트
        table.horizontalHeader().setStyleSheet(
            'QHeaderView::section { background-color: #2d5986; '
            'color: white; font-weight: bold; padding: 4px; }'
        )

        n = len(self.results)
        for rank, r in enumerate(self.results):
            # 행 배경: 민감도 크기에 따라 흰색 ~ 연한 빨강 그라데이션
            factor = rank / (n - 1) if n > 1 else 0.0  # 0=가장민감, 1=가장둔감
            bg = QColor(
                255,
                int(235 + factor * 20),
                int(235 + factor * 20)
            ...
            s_abs  = r['S_absolute']
            s_norm_str = f'{s_norm:.4f}' if np.isfinite(s_norm) else 'NaN'
            s_abs_str  = f'{s_abs:.3f}'  if np.isfinite(s_abs)  else 'NaN'

            table.setItem(rank, 0, cell(r['name']))
            table.setItem(rank, 1, cell(f"{r['value']:g} {r['unit']}",
                                        Qt.AlignRight))
            table.setItem(rank, 2, cell(f"±{r['tolerance']:.1f}%",
                                        Qt.AlignCenter))
            table.setItem(rank, 3, cell(s_abs_str, Qt.AlignRight))
            table.setItem(rank, 4, cell(s_norm_str, Qt.AlignRight))
            table.setItem(rank, 5, cell(str(rank + 1), Qt.AlignCenter))
    ...
    def _build_chart(self) -> FigureCanvasQTAgg:
        fig, ax = plt.subplots(figsize=(5, max(3, len(self.results) * 0.6 + 1)))
        canvas = FigureCanvasQTAgg(fig)

        # 플롯 데이터 (이미 |S_normalized| 내림차순 → 차트에서는 위가 가장 민감)
        names      = [r['name'] for r in self.results]
        s_norm     = [r['S_normalized'] for r in self.results]
        s_abs_vals = [r['S_absolute']   for r in self.results]

        # 차트에서 위→아래가 순위 1→N이 되도록 역순 배치
        names_rev  = list(reversed(names))
        s_norm_rev = list(reversed(s_norm))
        s_abs_rev  = list(reversed(s_abs_vals))

        colors = [
            '#d94f3d' if (np.isfinite(v) and v >= 0) else '#4f7fd9'
            for v in s_norm_rev
        ]

        # 수평 막대 그래프 (정규화 민감도)
        bars = ax.barh(names_rev, s_norm_rev, color=colors, alpha=0.85)

        # 막대 끝에 수치 레이블
        for bar, val in zip(bars, s_norm_rev):
        ...
        ax.set_title(
            f'Tornado Chart — Local Sensitivity @ {self.freq_mhz:.3f} MHz',
            fontsize=9, fontweight='bold'
        )
        ax.set_facecolor('white')
        ax.grid(True, axis='x', color='#E0E0E0', linestyle=':')

        # 두 번째 X축 (twiny): 절대 민감도 scatter 표시
        ax2 = ax.twiny()
        valid_abs = [(i, v) for i, v in enumerate(s_abs_rev)
                     if np.isfinite(v)]
        if valid_abs:
            y_positions = [i for i, _ in valid_abs]
            x_values    = [v for _, v in valid_abs]
            ax2.scatter(x_values, y_positions, marker='D',
                        color='#FF9800', s=30, zorder=5, label='Abs. S')
            ax2.set_xlabel('Absolute Sensitivity |∂Z/∂Xi| (Ω/unit)',
                           fontsize=8, color='#FF9800')
            ax2.tick_params(axis='x', colors='#FF9800', labelsize=7)

        fig.tight_layout()
        return canvas

    def closeEvent(self, event):
        """창 닫기 — 메인 윈도우에 영향 없음 (Non-modal 특성 유지)."""
        event.accept()
...
class CircuitCanvas(QWidget):
    ...
    def _edit_value(self, comp: Component):
        """소자 값 및 Tolerance 편집 다이얼로그를 열어 결과를 저장한다."""
        dlg = ComponentEditDialog(comp, self)
        if dlg.exec_() == QDialog.Accepted:
            comp.value = dlg.get_value()
            comp.tolerance = dlg.get_tolerance()
        ...
        for wire in self.wires:
            for side, pin_name in [(wire.start_comp_id, wire.start_pin),
                ...
                if side != comp.id:
                    continue
                # 이 와이어에서 상대방 핀 좌표
                other_id = (wire.end_comp_id if side == wire.start_comp_id
                            else wire.start_comp_id)
                other_pin = (wire.end_pin if side == wire.start_comp_id
                             else wire.start_pin)
...
# ---------------------------------------------------------------------------
# Circuit Widget (창1: CircuitCanvas + Sensitivity Analysis 버튼 컨테이너)
# ---------------------------------------------------------------------------

class CircuitWidget(QWidget):
    """CircuitCanvas를 감싸고 우측 상단에 Sensitivity Analysis 버튼을 배치하는 컨테이너."""
    ...
    def __init__(self, parent=None):
        ...
        # Sensitivity Analysis 버튼 (오버레이, 부모 위젯 기준 절대 위치)
        # Advanced 버튼과 동일한 스타일 상수(_ADV_BTN_STYLE) 사용
        self.sens_btn = QPushButton('Sensitivity Analysis', self)
        self.sens_btn.setFixedSize(170, 28)
        self.sens_btn.setStyleSheet(_ADV_BTN_STYLE)
        self._position_btn()
    ...
    def _position_btn(self):
        """Sensitivity Analysis 버튼을 우측 상단(margin 5px)에 위치."""
        self.sens_btn.move(self.width() - 170 - 5, 5)


# ---------------------------------------------------------------------------
# Smith Chart Widget (창2: SmithChartCanvas + Advanced 버튼 컨테이너)
# ---------------------------------------------------------------------------
...
class SmithChartWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.adv_btn.setStyleSheet(_ADV_BTN_STYLE)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._sensitivity_window: Optional[SensitivityResultWindow] = None
    ...
    def _build_ui(self):
        ...
        # CircuitWidget: CircuitCanvas + Sensitivity Analysis 버튼 컨테이너
        self.circuit_widget = CircuitWidget()
        self.circuit_widget.setMinimumWidth(480)
        self.circuit_canvas = self.circuit_widget.canvas  # 기존 참조 유지
        ...
        self.splitter.addWidget(self.circuit_widget)
        ...
        self.circuit_widget.sens_btn.clicked.connect(
            self.on_sensitivity_analysis_clicked)
    ...
    def on_sensitivity_analysis_clicked(self):
        """Sensitivity Analysis 버튼 클릭 처리."""
        # 회로 기본 검증 (소자 존재 및 값 입력 여부)
        comps = self.circuit_canvas.components
        if not comps:
            QMessageBox.warning(self, 'Sensitivity Analysis',
                                "배치된 소자가 없습니다.\n소자를 배치한 후 다시 시도하세요.")
            return
        missing = [c for c in comps if c.value is None]
        if missing:
            QMessageBox.warning(self, 'Sensitivity Analysis',
                                "값이 입력되지 않은 소자가 있습니다.\n"
                                "모든 소자를 더블클릭하여 값을 입력하세요.")
            return
        if len([c for c in comps if c.value is not None]) <= 1:
            QMessageBox.warning(self, 'Sensitivity Analysis',
                                "민감도 분석에는 값이 입력된 소자가 2개 이상 필요합니다.")
            return
        ...
        # 민감도 계산
        try:
            results = compute_local_sensitivity(
